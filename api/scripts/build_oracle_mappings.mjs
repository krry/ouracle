...
const TAROT_PATH = path.join('..', 'data', 'tarot_interpretations.json');
const ICHING_PATH = path.join('..', 'data', 'iChing_Legge.json');
const OUT_TAROT = path.join('data', 'tarot-octave-map.json');
const OUT_ICHING = path.join('data', 'iching-octave-map.json');
const OCTAVE_SCHEMA_PATH = path.join('data', 'octave-schema.json');
...
function loadOctaveSchema() {
  const raw = fs.readFileSync(OCTAVE_SCHEMA_PATH, 'utf8');
...
async function mapIChing() {
  ...
  const hexagrams = Array.isArray(raw) ? raw : (raw.hexagrams || []);
  const octaveTargets = buildOctaveTargets(loadOctaveSchema());
  const octaveEmbeddings = await embedTexts(octaveTargets.map((t) => t.signature_text));
  const targets = octaveTargets.map((t, i) => ({ ...t, embedding: octaveEmbeddings[i] }));
